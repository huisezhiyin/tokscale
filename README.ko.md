...
| <img width="48px" src=".github/assets/client-synthetic.png" alt="Synthetic" /> | [Synthetic](https://synthetic.new/) | 다른 소스에서 `hf:` 모델 접두사 또는 `synthetic` provider로 재분류 (+ Octofriend: `~/.local/share/octofriend/sqlite.db`) | ✅ 지원 |
...
- **멀티 플랫폼 지원** - OpenCode, Claude Code, Codex CLI, Cursor IDE, Gemini CLI, Amp, Droid, OpenClaw, Pi, Kimi CLI, Synthetic 사용량 통합 추적
...
- **키보드 내비게이션**:
  ...
  - `↑/↓`: 목록 탐색
  - `c/n/t`: 비용/이름/토큰별 정렬
  - `1-0`, `-`: 소스 토글 (OpenCode/Claude/Codex/Cursor/Gemini/Amp/Droid/OpenClaw/Pi/Kimi/Synthetic)
  - `p`: 9가지 색상 테마 순환
  - `r`: 데이터 새로고침
  - `e`: JSON으로 내보내기
  - `q`: 종료
...
- **연도 필터링**: 연도 간 탐색
- **소스 필터링**: 플랫폼별 필터 (OpenCode, Claude, Codex, Cursor, Gemini, Amp, Droid, OpenClaw, Pi, Kimi, Synthetic)
...
| Synthetic | 다른 소스에서 재분류 | 다른 소스에서 재분류 | `hf:` 모델 접두사 + `synthetic` provider 감지 |
...
Synthetic 사용량은 기존 에이전트 세션 파일의 **후처리**로 감지됩니다. [synthetic.new](https://synthetic.new/)를 API 게이트웨이로 사용한 메시지는 다음으로 식별됩니다:

- **모델 접두사**: `hf:` (예: `hf:moonshotai/Kimi-K2.5`), `accounts/fireworks/`, `accounts/together/`
- **Provider ID**: `synthetic`, `glhf`, `octofriend`

감지된 메시지는 원래 소스(예: OpenCode)에서 `synthetic` 소스로 재분류됩니다. 모델명은 정규화됩니다 (예: `hf:moonshotai/Kimi-K2.5` → `kimi-k2.5`).

**Octofriend**: Octofriend CLI 세션의 `~/.local/share/octofriend/sqlite.db`도 스캔합니다.
