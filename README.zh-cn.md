...
| <img width="48px" src=".github/assets/client-synthetic.png" alt="Synthetic" /> | [Synthetic](https://synthetic.new/) | 通过`hf:`模型前缀或`synthetic` provider从其他来源重新归类（+ Octofriend: `~/.local/share/octofriend/sqlite.db`） | ✅ 支持 |
...
- **交互式 TUI 模式** - 由 OpenTUI 驱动的精美终端 UI（默认模式）
  ...
  - 实时筛选和排序
  - 零闪烁渲染（原生 Zig 引擎）
- **多平台支持** - 跟踪 OpenCode、Claude Code、Codex CLI、Cursor IDE、Gemini CLI、Amp、Droid、OpenClaw、Pi、Kimi CLI 和 Synthetic 的使用情况
...
- **键盘导航**：
  ...
  - `1-0`、`-`：切换来源（OpenCode/Claude/Codex/Cursor/Gemini/Amp/Droid/OpenClaw/Pi/Kimi/Synthetic）
  - `p`：循环 9 种颜色主题
  - `r`：刷新数据
  - `e`：导出为 JSON
  - `q`：退出
...
- **年份筛选**：在年份之间导航
- **来源筛选**：按平台筛选（OpenCode、Claude、Codex、Cursor、Gemini、Amp、Droid、OpenClaw、Pi、Kimi、Synthetic）
...
| Synthetic | 从其他来源重新归类 | 从其他来源重新归类 | 检测`hf:`模型前缀 + `synthetic` provider |
...
Synthetic 使用量通过对现有代理会话文件的**后处理**来检测。使用 [synthetic.new](https://synthetic.new/) 作为 API 网关的消息通过以下方式识别：

- **模型前缀**：`hf:`（例如 `hf:moonshotai/Kimi-K2.5`）、`accounts/fireworks/`、`accounts/together/`
- **Provider ID**：`synthetic`、`glhf`、`octofriend`

这些消息从原始来源（例如 OpenCode）重新归类到 `synthetic` 来源。模型名称会被规范化（例如 `hf:moonshotai/Kimi-K2.5` → `kimi-k2.5`）。

**Octofriend**：同时扫描 `~/.local/share/octofriend/sqlite.db` 中的 Octofriend CLI 会话。
